<x-layout title="Create Hiking Trail">
    ...
    <div class="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
        <h2 class="text-2xl font-bold mb-6">Create a New Hiking Trail</h2>
        ...
        <form action="{{ route('trails.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="location" name="location" value="{{ old('location') }}" required class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                @error('location')
                <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="number" id="difficulty" name="difficulty" value="{{ old('difficulty') }}" required min="1" max="5" class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                @error('difficulty')
                <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" id="type_trail" name="type_trail" value="{{ old('type_trail') }}" required class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                @error('type_trail')
                <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
